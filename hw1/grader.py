...
def test3g():
    ...
    grader.require_is_equal(0, submission.compute_longest_palindrome_length(""))
    grader.require_is_equal(1, submission.compute_longest_palindrome_length("a"))
    grader.require_is_equal(2, submission.compute_longest_palindrome_length("aa"))
    grader.require_is_equal(1, submission.compute_longest_palindrome_length("ab"))
    grader.require_is_equal(3, submission.compute_longest_palindrome_length("animal"))


grader.add_basic_part('3g-0-basic', test3g, description='simple test')


def test3g(num_chars, length):
    import random
    random.seed(42)
    # Generate a random string of the given length
    text = ' '.join(chr(random.randint(ord('a'), ord('a') + num_chars - 1)) for _ in range(length))
    ans2 = submission.compute_longest_palindrome_length(text)


grader.add_hidden_part('3g-2-hidden', lambda: test3g(2, 10), max_points=1, max_seconds=1, description='random trials')
grader.add_hidden_part('3g-3-hidden', lambda: test3g(10, 10), max_points=1, max_seconds=1,
                       description='random trials (more characters)')
grader.add_hidden_part('3g-4-hidden', lambda: test3g(5, 20), max_points=1, max_seconds=1, description='random trials')
grader.add_hidden_part('3g-5-hidden', lambda: test3g(5, 400), max_points=1, max_seconds=2,
                       description='random trials (longer)')
