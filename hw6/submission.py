...
# Example where we add a variable to the csp
# csp = CSP()
...
#
# Notice that the third input to binary_factor is a function!
# factor_function should return 0 if the constraints are unsatisfied, and
# 1 or weight (for problem 2) otherwise.
# You can define factor_function with lambdas, or a nested function in python.
# The following examples functions corresponding to binary factors;
# unary factors have one variable
# Using lambdas
# csp.add_binary_factor(VAR1_NAME, VAR2_NAME, lambda x, y: x == y)
# Using nexted functions
# def are_equal(x,y):
#   return x == y
# csp.add_binary_factor(VAR1_NAME, VAR2_NAME, are_equal)
# see util.py for a nested function in action (are_equal)
...
def create_chain_csp(n: int) -> CSP:
    ...
    for j in range(1, len(variables)):
        csp.add_binary_factor(variables[j-1], variables[j], lambda x, y: x^y)
    # END_YOUR_CODE
    return csp
...
def create_nqueens_csp(n: int = 8) -> CSP:
    ...
    for variable in variables:
        csp.add_variable(variable, cols)
    for i in range(len(variables)):
        for j in range(i+1, len(variables)):
            csp.add_binary_factor(variables[i], variables[j], lambda c1, c2: c1 != c2 and abs(c1-c2) != abs(i-j))
    # END_YOUR_CODE
    return csp
...
class BacktrackingSearch:
    ...
    def get_unassigned_variable(self, assignment: Dict):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_CODE (our solution is 7 lines of code, but don't worry if you deviate from this)
            return min(filter(lambda var: var not in assignment, self.csp.variables), \
                key=lambda var: sum(self.satisfies_constraints(assignment, var, val) for val in self.domains[var]))
            # END_YOUR_CODE
