...
class ExactInference(object):
    ...
    def elapseTime(self) -> None:
        ...
        # BEGIN_YOUR_CODE (our solution is 6 lines of code, but don't worry if you deviate from this)
        nr, nc = (self.belief.getNumRows(), self.belief.getNumCols())
        nextBelief = util.Belief(nr, nc)
        for r in range(nr):
            for c in range(nc):
                nextBelief.setProb(r, c, sum(self.belief.getProb(r_t, c_t) * self.transProb[((r_t, c_t), (r, c))] \
                                            for r_t in range(nr) for c_t in range(nc) if ((r_t, c_t), (r, c)) in self.transProb))
...
class ParticleFilter(object):
    ...
    #             re-sample the particles from this distribution, choosing a new grid location for each of
    #             the |self.NUM_PARTICLES| new particles. To be extra clear: these new NUM_PARTICLES should be sampled
    #             from the new re-weighted distribution, not the old belief distribution.
    #
    # - agentX: x location of your car (not the one you are tracking)
    # - agentY: y location of your car (not the one you are tracking)
